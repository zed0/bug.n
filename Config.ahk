...
Config_init() {
	...
	Config_fontName			:= "Dejavu Sans Mono"			; A monospace font is preferable for bug.n to calculate the correct width of the bar and its elements (sub-windows).
	Config_fontSize			:= 8							; Font size in pixel.
	
	Config_normBgColor		:= "000000;252525;3a3a3a;4f4f4f;646464;797979;8e8e8e;a3a3a3;b8b8b8;cdcdcd"	; Normal background colour of bar elements; format: RRGGBB. The ten values are needed for indicating the number of windows per view (by default: darker colour means more windows) and distinct status information.
	Config_normFgColor		:= "ffffff;ffffff;ffffff;ffffff;ffffff;000000;000000;000000;000000;000000"	; Normal foreground colour of bar elements.
	Config_selBgColor		:= "2b5540;ebebeb;2b5540"		; Background colour of 'selected' or highlighted bar elements (format: RRGGBB). The first colour is used for the layout and command symbol, the second for selected tags and dereasing battery level and the third color is only used for alarming battery level.
	Config_selFgColor		:= "ebebeb;101010;ffffff"		; Foreground colour of 'selected' or highlighted bar elements.
	...
	; command gui
	Config_addRunCommands	:= "chrome"							; A semicolon separated list of commands, which can be run by AutoHotkey (i. e, "Run, <command>"), e. g. "cmd;explorer D:\;http://www.autohotkey.com"). Each command will be shown in the command GUI under the "Run" item.
	...
	Config_borderWidth		:= 1							; If > 0, the window border width is set to the integer value Config_borderWidth.
	...
	Config_showBorder		:= False							; If false, the window borders are hidden; therefor windows cannot be resized manually by dragging the border, even if using the according hotkey.
	Config_selBorderColor	:= "0x0000FF00"							; Border colour of the active window; format: 0x00BBGGRR (e. g. "0x006A240A", if = "", the system's window border colour is not changed).
	...
	Config_mouseFollowsFocus	:= False						; If true, the mouse pointer is set over the focused window, if a window is activated by bug.n.
	Config_shellMsgDelay		:= 100						; The time bug.n waits after a shell message (a window is opened, closed or the focus has been changed); if there are any problems recognizing, when windows are opened or closed, try to increase this number.
	...
	Config_rules[14]	:= "PuTTY;.*;;1;0;0;0;0;0"
	Config_rules[15]	:= "cygwin\/x.*;.*;;1;0;0;0;0;0"
	Config_rules[16]	:= "SpotifyMainWindow;.*;;1;0;256;0;0;0"
	Config_rulesCount	:= 16								; This variable has to be set to the total number of active rules above.
...
#F5::Run, explorer C:\
#F2::Manager_onShellMessage(2, 268435455)
;#Enter::Run, C:\cygwin\bin\run.exe -p /usr/X11R6/bin xterm -display 127.0.0.1:0.0 -ls
#Enter::Run, C:\cygwin\bin\run.exe -p /usr/X11R6/bin rxvt -display 127.0.0.1:0.0 -ls

#Tab::View_activateWindow(+1)			; Activate the last window selected
#j::View_activateWindow(+1)			; Activate the next window in the active view.
#k::View_activateWindow(-1)			; Activate the previous window in the active view.
#+j::View_shuffleWindow(+1)			; Move the active window to the next position in the window list of the view.
#+k::View_shuffleWindow(-1)			; Move the active window to the previous position in the window list of the view.
...
#f::Manager_maximizeWindow()			; Move and resize the active window to the size of the work area (only floating windows).
...
#Space::View_setLayout(">")				; Set the previously set layout. You may also use View_setLayout(">") for setting the next layout in the layout array.
...
#^Tab::Monitor_activateView(-1)	; Activate the previously activated view. You may also use Monitor_activateView(">") for activating the next / adjacent view.
...
#+`::Monitor_toggleBar()			; Hide / Show the bar (bug.n status bar) on the active monitor.
#`::Monitor_toggleTaskBar()			; Hide / Show the task bar.
